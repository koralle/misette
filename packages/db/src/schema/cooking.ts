...
export const cookingAttempt = sqliteTable(
  'cooking_attempt',
  {
    ...
    basedOnAttemptId: text('based_on_attempt_id').references((): AnySQLiteColumn => cookingAttempt.id, {
      onDelete: 'restrict',
    }),
  ...
  (table) => [
    uniqueIndex('cooking_attempt_id_recipeRevisionId_uidx').on(table.id, table.recipeRevisionId),
    index('cooking_attempt_recipeRevisionId_idx').on(table.recipeRevisionId),
    index('cooking_attempt_createdByUserId_cookedAt_idx').on(table.createdByUserId, desc(table.cookedAt)),
...
export const cookingAttemptStepNote = sqliteTable(
  ...
  (table) => [
    ...
    index('cooking_attempt_step_note_recipeStepId_recipeRevisionId_idx').on(table.recipeStepId, table.recipeRevisionId),
  ],
);

export const cookingAttemptRelations = relations(cookingAttempt, ({ one, many }) => ({
...
export const cookingAttemptStepNoteRelations = relations(cookingAttemptStepNote, ({ one }) => ({
  attempt: one(cookingAttempt, {
    fields: [cookingAttemptStepNote.cookingAttemptId, cookingAttemptStepNote.recipeRevisionId],
    references: [cookingAttempt.id, cookingAttempt.recipeRevisionId],
  }),
  step: one(recipeStep, {
    fields: [cookingAttemptStepNote.recipeStepId, cookingAttemptStepNote.recipeRevisionId],
...
export type CookingAttemptStepNoteInsert = typeof cookingAttemptStepNote.$inferInsert;
